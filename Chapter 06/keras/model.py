...
def train(conv1_size, conv2_size, dropout, hidden1_size, optimizer, log_learning_rate, epochs):
    ...
    model.add(
        tf.keras.layers.Conv2D(
            filters=conv1_size, kernel_size=(3, 3), activation="relu", input_shape=x_train.shape[1:]
        )
    )
    model.add(tf.keras.layers.Conv2D(filters=conv2_size, kernel_size=(3, 3), activation="relu"))
...
if __name__ == "__main__":
    ...
    # Polyaxon
    tracking.log_metrics(eval_accuracy=eval_accuracy)
